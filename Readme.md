...
### Picframe : an e-ink picture frame powered by a raspberry pi (2022)

More details to come soon! As my girlfriend is working in the north for a few months, I built a pair of intelligent e-ink pictures frames to exchange daily pictures. They pull the latest image received in a Gmail account. The e-ink screen consumes very little energy and displays images with small filesizes, which is perfect for the slow connection she is experiencing in Nunavik. The frame's job is to fetch an image file from a specific URL, generated by a dockerized Flask App hosted on the Google Cloud Platform. 
